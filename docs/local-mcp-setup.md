# CreNexus 本地 MCP 接入说明

CreNexus 当前已经提供真正 MCP stdio server：Codex、Cursor、Claude Code 或其他 MCP 客户端可以通过同一套 tools 查看各软件桥状态，并调用首批安全只读/受保护 DXF 工具。
...
| CreNexus MCP | 通过 stdio 暴露 `tools/list` 和 `tools/call`，把 status、probe、dry-run、evidence 和 confirmed sandbox 写入做成结构化工具。 |
