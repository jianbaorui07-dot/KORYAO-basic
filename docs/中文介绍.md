...
> English name: **CreNexus Trinity Protocol**

创枢三联主推“不用图像描摹的精确图片转 SVG / Illustrator AI”：原始 RGBA 像素被重建为矩形复合路径，验证无嵌入位图后再由 Illustrator 存储为 AI。项目同时保留完整的 Windows-first 本地桥接框架：Codex skill 负责选择路线，CreNexus MCP stdio server 注册安全 tools，Adobe UXP / 本地代理连接桌面软件；ComfyUI、Photoshop、CAD、Blender、剪映 / CapCut 等能力继续存在。

人话说：Codex 不直接乱碰 PSD、DWG、模型、草稿和客户素材。它先用 skill 判断路线，再问 CreNexus MCP“这台机器准备好了吗、能安全做什么”，需要 Adobe 插件链路时再走 UXP / Node Proxy；ComfyUI、CAD、Photoshop、Illustrator、Blender、剪映 / CapCut 继续负责真正生成、渲染、修图、矢量化、制图和剪辑。
