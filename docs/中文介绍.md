...
星桥三联不是“已经完整接管所有创意软件”的产品，而是一个 Windows-first 的本地桥接框架。它现在明确服务三层：第一层是 Codex skill，用来选择软件桥、读取正确文档、执行安全验证；第二层是 StarBridge MCP stdio server，把已经安全收敛的能力注册成 tools，供 Codex、Cursor、Claude Code 等 AI 助手调用；第三层是 Adobe UXP / 本地代理，用来把 Photoshop 等桌面软件的可审计动作接到 MCP 后面。
...
| --- | --- | --- | --- | --- |
| ComfyUI | `comfyui.system_probe`、`comfyui.workflow_validate`、`comfy.workflow_lifecycle_summary`、txt2img API 示例 | Python 3.10+；本机 ComfyUI server；至少一个 checkpoint | 不能提交模型或生成图；`img2img`、inpaint、upscale 尚未真实闭环 | 补 lint/repair、probe gate 和 queue payload dry-run |
