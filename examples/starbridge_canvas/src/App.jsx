...
const CANVAS_LIVE_ENDPOINT = '/api/canvas-live'
const SELECTION_ENDPOINT = '/api/selection'
const VIEW_STATE_ENDPOINT = '/api/view-state'
const SELECTION_STATE_ELEMENT_ID = 'starbridge-canvas-selection-state'
...
const ANNOTATION_TOOL_ID = 'starbridge-annotation'
...
const cowartUiOverrides = {
  translations: {
    en: {
      ...
      'tool.starbridge-annotation': ANNOTATION_TOOL_LABEL
    },
    'zh-cn': {
      'tool.ai-image': AI_IMAGE_HOLDER_LABEL,
      'tool.starbridge-annotation': ANNOTATION_TOOL_LABEL
  ...
  tools(editor, tools) {
    return {
      ...
      [AI_IMAGE_TOOL_ID]: {
        ...
        meta: {
          starbridgeCanvasTool: 'ai-image-holder'
        }
      },
      [ANNOTATION_TOOL_ID]: {
        id: ANNOTATION_TOOL_ID,
        label: 'tool.starbridge-annotation',
        ...
        meta: {
          starbridgeCanvasTool: 'annotation'
        }
      }
    }
  }
...
function incrementCount(counts, record) {
  const typeName = record?.typeName ?? 'record'
...
function summarizeStoreChanges(changes) {
  const added = Object.values(changes.added ?? {}).map(summarizeRecord).filter(Boolean)
  const updated = Object.values(changes.updated ?? {})
    .map(([_previous, next]) => summarizeRecord(next))
    .filter(Boolean)
  const removed = Object.values(changes.removed ?? {}).map(summarizeRecord).filter(Boolean)
  const counts = {}

  for (const record of [...added, ...updated, ...removed]) {
    incrementCount(counts, record)
  }
  ...
  return {
    ...
    changedRecordCount: added.length + updated.length + removed.length
  }
}

function mergeRecordGroups(left = {}, right = {}) {
  return {
    added: [...(left.added ?? []), ...(right.added ?? [])].slice(-20),
    updated: [...(left.updated ?? []), ...(right.updated ?? [])].slice(-20),
    removed: [...(left.removed ?? []), ...(right.removed ?? [])].slice(-20)
  }
}

function mergeLiveSummaries(left, right) {
  if (!left) return right

  const counts = { ...left.counts }
  for (const [key, value] of Object.entries(right.counts ?? {})) {
    counts[key] = (counts[key] ?? 0) + value
  }

  return {
    counts,
    records: mergeRecordGroups(left.records, right.records),
    changedRecordCount: left.changedRecordCount + right.changedRecordCount
  }
}
...
function liveEventLabel(event) {
  if (!event) return '等待绘制'

  const total = Object.values(event.counts ?? {}).reduce((sum, count) => sum + count, 0)
  if (total === 0) return '画布已同步'

  const mainType = Object.keys(event.counts ?? {})[0] ?? '对象'
  return `刚更新 ${total} 个${mainType}`
}
...
export default function App() {
  ...
  const handleMount = useCallback((editor) => {
    window.__starbridgeCanvasEditor = editor
    window.__starbridgeCanvasSelection = () => getCowartSelection(editor)
    window.__starbridgeCanvasViewState = () => getCowartViewState(editor)
    ...
    let pendingLiveSummary = null
    let liveEventTimer = null

    async function publishLiveEvent(summary) {
      if (!summary || summary.changedRecordCount === 0) return

      try {
        const response = await fetch(CANVAS_LIVE_ENDPOINT, {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
            kind: 'drawing-change',
            actor: 'starbridge-canvas-browser',
            source: 'user',
        ...
        if (!response.ok) {
          throw new Error(`Failed to publish live canvas event: ${response.status}`)
        }
      } catch (error) {
        console.error(error)
      }
    ...
    function queueLiveEvent(summary) {
      pendingLiveSummary = mergeLiveSummaries(pendingLiveSummary, summary)
      if (liveEventTimer) return

      liveEventTimer = window.setTimeout(() => {
        const summaryToPublish = pendingLiveSummary
        pendingLiveSummary = null
        liveEventTimer = null
        publishLiveEvent(summaryToPublish)
      }, 120)
    }
    ...
    const unsubscribe = editor.store.listen(
      ({ changes }) => {
        scheduleSave()
        queueLiveEvent(summarizeStoreChanges(changes))
      },
      {
      source: 'user',
      scope: 'document'
    ...
    if ('EventSource' in window) {
      ...
      canvasEvents.addEventListener('drawing-event', (event) => {
        try {
          setLastLiveEvent(JSON.parse(event.data))
        } catch (error) {
          console.warn('CreNexus Canvas live event could not be parsed.', error)
        }
      })
      canvasEvents.onerror = (error) => {
        setLiveConnectionState('reconnecting')
        console.warn('CreNexus Canvas live refresh disconnected.', error)
      }
    } else {
    ...
    return () => {
      ...
      remoteLoadController?.abort()
      canvasEvents?.close()
      if (window.__starbridgeCanvasEditor === editor) {
        delete window.__starbridgeCanvasEditor
        delete window.__starbridgeCanvasSelection
        delete window.__starbridgeCanvasViewState
      }
  ...
  return (
    <main className="cowart-canvas" aria-label="CreNexus infinite canvas">
      <div className="starbridge-live-status" role="status" aria-live="polite">
        ...
        <span className="starbridge-live-event">{liveEventLabel(lastLiveEvent)}</span>
      </div>
      <Tldraw
