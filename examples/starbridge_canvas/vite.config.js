...
const projectDir = resolve(process.env.STARBRIDGE_CANVAS_PROJECT_DIR ?? process.env.COWART_PROJECT_DIR ?? process.cwd())
const canvasDir = resolve(
  process.env.STARBRIDGE_CANVAS_DIR ?? process.env.COWART_CANVAS_DIR ?? join(projectDir, 'canvas')
)
const canvasFile = join(canvasDir, 'starbridge-canvas.json')
const legacyCanvasFile = join(canvasDir, 'cowart-canvas.json')
const selectionFile = join(canvasDir, 'starbridge-selection.json')
const viewStateFile = join(canvasDir, 'starbridge-view-state.json')
...
const canvasFileName = 'starbridge-canvas.json'
...
function sendServerSentEvent(res, eventName, payload) {
  res.write(`event: ${eventName}\n`)
  res.write(`id: ${payload.version}\n`)
  res.write(`data: ${JSON.stringify(payload)}\n\n`)
}

function broadcastCanvasEvent(eventName, payload) {
  for (const client of canvasEventClients) {
    ...
    try {
      sendServerSentEvent(client, eventName, payload)
    } catch {
      canvasEventClients.delete(client)
    }
  }
...
function broadcastCanvasChanged(result) {
  broadcastCanvasEvent('canvas-changed', {
...
function sanitizeCountMap(value) {
  ...
  for (const [key, count] of Object.entries(value)) {
    ...
    counts[truncateString(key, 48)] = Math.min(count, 999)
  }
  return counts
}

function sanitizeLiveRecords(value) {
  if (!value || typeof value !== 'object') return {}

  const records = {}
  for (const [group, items] of Object.entries(value)) {
    if (!Array.isArray(items)) continue
    records[truncateString(group, 32)] = items.slice(0, 20).map((item) => ({
      id: truncateString(item?.id, 120),
      typeName: truncateString(item?.typeName, 48),
      type: truncateString(item?.type, 48)
    }))
  }
  return records
}

function sanitizeLiveEvent(value) {
  if (!value || typeof value !== 'object') return null

  return {
    version: ++canvasEventVersion,
    updatedAt: new Date().toISOString(),
    kind: truncateString(value.kind, 48) ?? 'canvas-change',
    actor: truncateString(value.actor, 80) ?? 'browser',
    source: truncateString(value.source, 48) ?? 'user',
    pageId: truncateString(value.pageId, 120),
    counts: sanitizeCountMap(value.counts),
    records: sanitizeLiveRecords(value.records)
  }
}

function broadcastDrawingEvent(payload) {
  broadcastCanvasEvent('drawing-event', payload)
}
...
async function readPageSnapshots() {
  ...
  for (const entry of entries) {
    ...
    const legacyFilePath = join(canvasPagesDir, entry.name, legacyCanvasFileName)
    try {
      ...
      try {
        const snapshot = await readJsonFile(legacyFilePath)
        if (isSnapshot(snapshot)) snapshots.push({ filePath: legacyFilePath, snapshot })
      } catch (legacyError) {
...
async function loadCanvasSnapshot() {
  ...
  try {
    ...
    try {
      return {
        snapshot: await readJsonFile(legacyCanvasFile),
        path: legacyCanvasFile,
        storage: 'legacy-cowart-single-file'
      }
    } catch (legacyError) {
...
function canvasStoragePlugin() {
  return {
    name: 'starbridge-canvas-storage',
    configureServer(server) {
      ...
      server.middlewares.use('/api/canvas-live', async (req, res) => {
        try {
          if (req.method !== 'POST') {
            res.statusCode = 405
          ...
          if (!eventPayload) {
            sendJson(res, 400, { error: 'Expected a CreNexus Canvas live event.' })
            return
          }

          broadcastDrawingEvent(eventPayload)
        ...
        try {
          ...
          if (req.method === 'PUT') {
            ...
            if (!isSelectionState(selection)) {
              sendJson(res, 400, { error: 'Expected a CreNexus Canvas selection state.' })
              return
            }
        ...
        try {
          ...
          if (req.method === 'PUT') {
            ...
            if (!isViewState(viewState)) {
              sendJson(res, 400, { error: 'Expected a CreNexus Canvas view state.' })
              return
            }
